	...
	$.ajax({
		...
		if(status == "success"){ 
			 if(data == 'erro'){ 
				 ...
				 localStorage.setItem("session",  data); //title.text());	        	
	...
	$.ajax({
		...
		if(status == "success"){ 
			if(data.erro !== undefined){ 
				...
				if(tp == 1){
					myApp.alert("Boleto enviado com sucesso! ","Atenção!", function(){
						buscaServicos();
					});
...
/* Boleto */
$$(document).on('click', '#novoBoleto', function (e) {
	console.log('Boleto');
	mainView.router.loadPage('servicos-pagboleto.html');
	
	myApp.onPageInit('servicos-pagboleto', function (page) {
		var servicovalor 	= localStorage.getItem('servicovalor');		
		$(".servico-valor").html("R$"+number_format(servicovalor,2,",","."));
	});
});
